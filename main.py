...
robot_FreeBuf = freebuf.FreeBuf('KeyWords.txt' , './Events/EventsIDFreeBuf.txt')
...
def runFreeBuf():
    print('*'*15+'FreeBuf start Running! The time is: %s' % datetime.now())
    robot_FreeBuf.keyWordscheck(robot_FreeBuf.dataAchieve(robot_FreeBuf.dataRequest()))
    print('*'*15+'FreeBuf Running over! The time is: %s' % datetime.now())
